...
功能：LINE Webhook + 問診流程 + 完整記錄 + Export API + LLM 回覆
"""
import os
import hmac
import hashlib
...
請用溫和、專業的語氣回答。
如果用戶問的是症狀評估，請引導他們輸入「泌尿道問題」來進行完整問卷。
如果不知道答案，建議他們就醫。"""

def llm_chat(user_input: str) -> Optional[str]:
    """使用 OpenAI Chat Completions REST API 回覆用戶問題"""
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[DEBUG] LLM HTTP error: {resp.status_code} {resp.text[:300]}")
            return None
        data = resp.json()
...
def is_survey_keyword(text: str) -> bool:
    """檢查是否為問卷觸發關鍵字"""
...
async def lifespan(app: FastAPI):
    ...
    db.init_db()
    flow.load()
    print(f"Flow loaded: {len(flow.nodes)} nodes")
    yield
    # 關閉時清理
    db.close()
...
async def webhook(
        ...
        if event.get("type") == "follow":
            ...
            if user_id and reply_token:
                ...
                if welcome_node:
                    ...
                    replies = flow.build_reply(welcome_node)
                    line_reply(reply_token, replies)
                    db.log_message(
                        user_id=user_id,
                        node_id="0",
                        symptom_code=welcome_node.get("tags", {}).get("code", ""),
                        action_tag=welcome_node.get("tags", {}).get("action_tag", ""),
                        user_input="[加入好友]",
                        bot_reply="\n".join(replies),
                        prompt=welcome_text,
                        education_text=welcome_node.get("education_text", ""),
                        intent="welcome",
                        is_end=False
                    )
                    # 設定到第 0 題，讓用戶下一句可以回答第 0 題
    ...
    for event in data.get("events", []):
        ...
        # current_node_id == "0" 視為正常問卷第一個問題，不再強制跳題
